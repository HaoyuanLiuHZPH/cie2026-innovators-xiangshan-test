...
| **关卡** | **条件变量** | **含义** | **比喻** |
| --- | --- | --- | --- |
| **allowDispatch** | `**allowDispatch(i)**` | LSQ 有空间容纳本条访存指令 | 仓库满了就别再往里塞 |
| **uopBlockByIQ** | `**uopBlockByIQ(i)**` | 目标 IQ 有空闲入队口 | 传送带满了就等着 |
| **thisCanActualOut** | `**thisCanActualOut(i)**` | 前方无阻塞、ROB 可接收 | 跑道清空了才能起飞 |
...
`**allowDispatch**` 是一条**链式传递**的信号——第 `**i**` 条 uop 的 `**allowDispatch**` 依赖于第 `**i-1**` 条。这就像排队买票，前面的人买完了才轮到你。
...
这个信号判断**目标 IQ 的入队口是否被占满**。每个 IQ 有 `**numEnq**` 个入队口，如果分配给该 IQ 的 uop 数量超过了 `**numEnq**`，超出的 uop 就会被阻塞：
...
某些指令被标记为 `**waitForward**`，意味着它需要等待前序写回结果。如果 ROB 不为空（说明前面还有未提交的指令），这类指令就必须等待：
...
某些指令被标记为 `**blockBackward**`，意味着它会**阻止后面的指令通过**。典型的例子是 **fence** 和 **fence.i** 指令：
...
### 6.5.3 IQ 有效条目数的延迟补偿
另一个关键优化是 `**enableDispatchIQBalanceOpt**`。当该选项开启时，Dispatch 会**预估当前周期正在入队的 uop 数量**，将其加入 IQ 计数中：
...
:::warning
❤️新手建议  
Timing Pressure 的优化细节较为复杂，初学者只需理解核心思想：**用"预估+分段"换"全量+实时"**，在精度和时序之间取折衷。具体的参数（如 `**updateInterval = 3**`、补偿偏移 `**+6**`）属于工程调优的范畴，可在后续深入学习中逐步掌握。
