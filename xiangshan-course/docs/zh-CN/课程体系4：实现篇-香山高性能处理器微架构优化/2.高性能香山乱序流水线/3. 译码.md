...
香山用 `**XSDecode**` 类来描述一条指令的所有译码控制信号。（定义于 `src/main/scala/xiangshan/backend/decode/DecodeUnit.scala`第94-110行附近）
...
### 3.2.4 默认值——查不到怎么办？
如果一条指令的编码**匹配不到任何合法条目**，就会命中默认值 `**decodeDefault**`，其 `**selImm**` 被设为 `**INVALID_INSTR**`，触发**非法指令异常**：
...
:::danger
_**这就像翻译官遇到了一个不认识的词——既然看不懂，就标记为"异常"，交给更高级的处理机制。**_
...
| **译码表对象** | **覆盖范围** | **对应源码** |
| --- | --- | --- |
| `**XDecode**` | RV32/64I、M 扩展、系统指令 | DecodeUnit.scalaL133 |
| `**FDecode**` | 浮点 F/D 扩展 | DecodeUnit.scala |
| `**VecDecoder**` | 向量 V 扩展（OPIVV/OPIVX/OPMVV 等） | VecDecoder.scalaL186 |
| `**BitmanipDecode**` | B 扩展位操作 | DecodeUnit.scala |
| `**ScalarCryptoDecode**` | 标量加密扩展 | DecodeUnit.scala |


各表独立定义，在 `**DecodeUnit**` 中合并为一张完整的查找表，进行并行匹配。
...
### 3.3.2 预译码提取了什么？
这个阶段不会进行完整的译码，而是生成一个名为 `**PreDecodeInfo**` 的轻量级信息包。这个信息包就像是给后续流水线的一个“速查摘要”，专门用于提前判断和处理可能改变程序执行流程的指令。

| **提取的字段** | **作用与含义** |
| :--- | :--- |
| `**valid**` | **指令有效性标志**。快速判断当前读取的比特位是否构成一条合法的指令。如果无效，可提前触发异常。 |
| `**isRVC**` | **压缩指令标识**。判断该指令是标准的32位指令，还是16位的RISC-V压缩指令（RVC）。这直接影响下一条指令的地址计算（+4或+2）。 |
| `**brAttribute**` | **分支属性**。标识指令的类型，特别是它如何改变控制流：   • 无条件跳转（如 `JAL`）   • 间接跳转（如 `JALR`）   • 条件分支（如 `BEQ`、`BNE`）   • 非分支指令 |
| `**jumpOffset**` | **跳转偏移量**。对于像 `JAL`和条件分支这类带有立即数偏移的指令，预译码会提前提取出这个偏移量，用于**快速计算潜在的目标地址**（Target PC = Current PC + Offset）。 |


### 3.3.3 预译码的分支匹配表
在预译码阶段，核心任务之一是**快速、准确地识别出所有可能改变程序执行流向的指令**，即分支/跳转指令。为了高效完成此任务，香山处理器使用了一张预先定义好的 `**brTable**`**（分支匹配表）**。
...
| :--- | :--- | :--- | :--- |
| `**None**` | **非分支指令**。程序顺序执行下一条。 | `EBREAK`, `ADD`, `LW` | 无需特殊处理，继续顺序取指。 |
| `**Direct**` | **无条件直接跳转**。目标地址由当前PC+偏移量**直接计算**得出。 | `JAL`, `C.J` | 预译码需提取`jumpOffset`，分支预测器可**直接算出**目标地址。 |
| `**Indirect**` | **间接跳转**。目标地址来自于**寄存器的值**，运行时才能确定。 | `JALR`, `C.JALR` | 目标地址未知。 |
| `**Conditional**` | **条件分支**。是否跳转取决于**条件判断结果**。 | `BEQ`, `BNE`, `C.BEQZ` | 预译码需提取偏移量。预测器需进行**方向预测**（跳转/不跳转）和**目标地址预测**。 |
...
:::danger
_**Pre-Decode 像海关安检——快速扫一眼护照，判断你是不是"需要特别关注的人"；Decode 像入境审查——详细审核你的签证、行程、资金，决定你能做什么。**_
...
这两条指令的**组合语义**等价于一条 `**ADD.UW r1, r0, zero**`（即伪指令 `**zext.w**`）。但从硬件视角看，它们却被当作两个独立的任务来处理。
...
_Fusion 通过识别并合并这些“编译器生成的、语义上可合并的指令对”，_将多条指令转换为一条更复杂的微操作（Micro-op）_，从而显著减轻后端流水线的资源压力，提升整体执行吞吐率和能效。_
...
:::danger
_**这就像超市的"买一送一"——只有**__**紧挨着**__**的两件商品才能凑单，隔了一个货架的不算。**_
...
:::danger
_**第一条指令是"生产者"，第二条是"消费者"，融合的前提是它们之间存在明确的供需关系。**_
...
| **模式** | **说明** | **例子** |
| --- | --- | --- |
| **融合为已有指令** | 第一条的 op 替换为另一条合法指令的译码结果 | `**SLLI+ADD**`→ `**SH1ADD**` |
| **融合为自定义操作码** | 第一条的 fuOpType 替换为香山自定义的内部操作码 | `**SLLI32+SRLI31**` → `**szewl1**` |
...
### 3.4.3 Fusion 的工作机制
`**FusionDecoder**` 模块对译码宽度内每对相邻指令**并行检测所有融合模式**，优先级由列表顺序决定：
...
当某种融合匹配成功时，`**FusionDecodeReplace**` 结构携带需要替换的控制信号，更新到第一条指令上，同时将第二条指令标记为"清除"：
...
| **融合模式** | **源指令对** | **融合结果** | **典型用途** |
| --- | --- | --- | --- |
| 零扩展字 | `**SLLI 32**`+`**SRLI 32**` | `**ADD.UW**`（zext.w） | 32→64 位零扩展 |
| 零扩展半字 | `**SLLI 48**`+`**SRLI 48**` | `**PACKW**`（zext.h） | 16→64 位零扩展 |
| 符号扩展半字 | `**SLLIW 16**`+`**SRAIW 16**` | `**SEXT.H**` | 16→32 位符号扩展 |
| 移位加 | `**SLLI n**`+`**ADD**` | `**SHnADD**`（n=1,2,3,4） | 地址计算 |
| 右移加 | `**SRLI n**`+`**ADD**` | `**SRnADD**`（n=29,30,31,32） | 高位提取+加法 |
| 奇数加 | `**ANDI 1**`+`**ADD/W**` | `**ODDADD/ODDADDW**` | 对齐计算 |
| 加法取字节 | `**ADDW**`+`**ANDI 0xFF**` | `**ADDWBYTE**` | 字节提取 |
| 加法取位 | `**ADD**`+`**ANDI 0x1**` | `**ADDWBIT**` | 位提取 |
| 逻辑取 LSB | 逻辑指令 + `**ANDI 1**` | `**logiclsb**` | 逻辑运算后取最低位 |
| 32位立即数 | `**LUI**`+`**ADDI/W**` | `**lui32add/lui32addw**` | 大立即数构造 |
| 7位乘法 | `**ANDI 127**`+`**MULW**` | `**mulw7**` | 小范围乘法优化 |
...
### 3.5.1 为什么需要拆分？
RISC-V 中有些指令的语义**无法在一个微操作中完成**。最典型的就是**向量指令**——一条 `**VADD.VV**` 可能同时操作 1 组甚至 8 组向量寄存器（取决于 LMUL），但后端执行单元每次只能处理有限数量的元素。

:::danger
_**比喻：**__** 一条向量指令就像一个"团购订单"——一份订单可能包含 8 组同样的商品，但仓库每次只能发一组。所以必须把一份订单**__**拆成 8 个子订单**__**，逐个发货。**_
...
| **类型** | **映射关系** | **处理模块** | **例子** |
| --- | --- | --- | --- |
| **简单指令** | 1 条宏指令 → 1 条微操作 | `**DecodeUnit**`（简单译码器） | 标量 ADD、SUB、LW |
| **复杂指令** | 1 条宏指令 → N 条微操作 | `**DecodeUnitComp**`（复杂译码器） | 向量 VADD、vsetvli、VLSE8 |


判断标准由 `**uopSplitType**` 字段决定——如果它不等于 `**UopSplitType.X**`（即不是"无需拆分"），就是复杂指令。
...
**核心思想：** 复杂译码器 `**DecodeUnitComp**` 的结果被排在输出序列的**前面**，简单译码器的结果排在**后面**。这就像超市收银——买了很多东西的顾客（复杂指令拆出的多个 uop）先过秤，买了一件的顾客（简单指令）紧随其后，保证程序顺序不乱。

### 3.5.4 拆分类型（UopSplitType）一览
| **UopSplitType** | **含义** | **典型指令** | **拆分数量** |
| --- | --- | --- | --- |
| `**X**` | 无需拆分 | 标量 ADD, LW | 1 |
| `**VSET**` | vset 配置指令 | vsetvli, vsetvl | 2（配置 + 写回） |
| `**VEC_VVV**` | 向量 OPIVV | VADD.VV | LMUL |
| `**VEC_VXV**` | 向量 OPIVX/OPIVI | VADD.VX, VADD.VI | LMUL |
| `**VEC_0XV**` | 向量（0号源特殊） | VMACC.VV | LMUL |
| `**VEC_US_LDST**` | 向量 unit-stride/strided load/store | VLE8.V, VLSE8.V | EMUL×(NF+1) |
| `**VEC_IX_LDST**` | 向量 indexed load/store | VLUXEI8.V | Max(LMUL,EMUL)×(NF+1) |
| `**VEC_RGATHER**` | vrgather | VRGATHER.VV | 按LMUL和SEW计算 |
| `**VEC_COMPRESS**` | vcompress | VCOMPRESS | 按LMUL计算 |
| `**VEC_SLIDE**` | vslide | VSLIDEUP | 按LMUL计算 |


### 3.5.5 UopInfoGen——计算"要拆成几个"
`**UopInfoGen**` 模块就像一个**计算器**——根据 `**uopSplitType**` 和当前 VType 配置（LMUL、SEW、EMUL），精确算出每条指令需要拆成几个微操作：
...
| **指令** | **拆分方式** | **原因** |
| --- | --- | --- |
| `**vsetvli**` | 2 uop：① 配置 VType ② 写回 VL 到 x 寄存器 | 需要同时完成"改配置"和"返回值"两件事 |
| `**vsetvl**` | 2 uop：同上 | 同上 |
...
**OPIVI 类型**（如 `**VADD.VI**`，向量 + 立即数）：

+ 与 OPIVX 类似，只是 src1 变成立即数
+ 同样使用 `**UopSplitType.VEC_VXV**`

_**比喻：标量-向量混合拆分就像"一个老师（标量）给多个班级（向量分组）上课"——老师只有一个，但每个班级都需要单独上一遍。**_
...
1. `**s_idle**`：等待复杂指令到来
2. 检测到 `**isComplex**` 信号 → 锁存指令，进入 `**s_active**`
3. `**s_active**`：每拍输出一个 uop，同时递增 `**numDecodedUop**`，递减 `**uopRes**`
4. 每个 uop 自动携带：
...
5. `**uopRes**` 归零 → 回到 `**s_idle**`

### 3.5.8 Segment 指令的拆分（进阶）
Segment load/store（NF > 0，即一次操作多个字段）的拆分更为复杂，uop 数量为 `**EMUL × (NF + 1)**`：
