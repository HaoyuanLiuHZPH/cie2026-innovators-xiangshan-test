[为什么需要超标量？——从性能瓶颈到架构革新.pptx](https://bosc.yuque.com/attachments/yuque/0/2026/pptx/66996431/1778553766938-2391c117-d300-4fa0-b155-8ccea9eb155d.pptx)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1778228172505-8f10db01-5b5e-4a6d-85ad-6cf5e133d652.png)
...
<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">考虑如下 RISC-V 指令序列（计算</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">a = b + c; d = a + e</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">）：</font>
...
+ <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">第 4 周期：</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">add x3, x1, x2</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);"> 进入执行阶段（EX），但</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">add x5, x3, x4</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);"> 若想在第 5 周期进入 ID 阶段，会发现</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">x3</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">尚未写回（WB 阶段在第 6 周期）—— 这是</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">RAW 数据冲突</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">（写后读）。</font>
+ <font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">为了保证结果正确，流水线必须停顿 1 个周期，等待</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">x3</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">写回后，</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">add x5</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">才能继续执行。</font>
...
<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">假设计算</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">C = A × B</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">（2×2 矩阵），核心指令序列（简化）：</font>
...
```
 ...
 在单发射流水线中，`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">mul x1</font>`和`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">mul x2</font>`必须串行执行；但这两条乘法指令无任何数据相关性，完全可以并行执行 —— 超标量架构正是利用这种 “指令级并行（ILP）”，将多条无冲突指令在同一周期发射到多个 ALU / 乘法器执行，大幅提升吞吐量。  
