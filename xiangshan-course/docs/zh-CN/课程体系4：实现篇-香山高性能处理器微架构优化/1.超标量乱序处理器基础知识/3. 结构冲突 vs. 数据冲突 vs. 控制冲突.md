[流水线冒险：结构、数据与控制三大冲突深度解析.pptx](https://bosc.yuque.com/attachments/yuque/0/2026/pptx/66996431/1778555222380-78bef6cf-d09f-49e4-9c4c-241fb45db6d9.pptx)
...
1. **<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">最经典的指令 / 数据内存冲突</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">冯・诺依曼架构（指令和数据存在同一块内存）中，IF 级的取指指令和 MEM 级的访存指令，会在同一周期争抢同一个内存端口 / 总线。比如周期 4 时，</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">lw</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">指令在 MEM 级读数据内存，同时第 4 条指令在 IF 级取指令，二者争抢同一个内存，直接触发结构冲突。</font><font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">哈佛架构（指令内存、数据内存物理分离）就是为了彻底解决这个结构冲突，也是所有流水线处理器的标配设计。</font>
...
1. **<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">Load-Use 冒险（最特殊、必须插入暂停的 RAW 冲突）</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">就是你之前图中的核心场景：</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">lw $s0, 40($0)</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);"> 指令要到</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">MEM 级结束时</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">才能从数据内存中拿到正确的</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">$s0</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">值，但后续的</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">and $t0, $s0, $s1</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">在</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">EX 级前半周期</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">就要用</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">$s0</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">做运算，时序完全错位，哪怕用数据转发也无法解决，必须插入 1 个周期的暂停，是唯一一类无法仅靠转发解决的数据冲突。</font>
2. **<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">1 周期延迟的算术指令 RAW 冲突</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">前序是算术指令</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">add $s0, $s1, $s2</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">（EX 级结束就能拿到运算结果），后序是</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">and $t0, $s0, $s3</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">，后序 EX 级要用</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">$s0</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">，前序 EX 级才出结果，差 1 个周期，是最常见的 RAW 冲突。</font>
3. **<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">名相关导致的 WAR/WAW 冲突</font>**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">比如两条指令</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">add $s0, $s1, $s2</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">和</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">addi $s0, $s3, 1</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">，都写同一个架构寄存器</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">$s0</font>`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">，若后序指令提前写回，就会导致 WAW 冲突；乱序执行中这类冲突非常普遍，是寄存器重命名技术的核心优化目标。</font>
