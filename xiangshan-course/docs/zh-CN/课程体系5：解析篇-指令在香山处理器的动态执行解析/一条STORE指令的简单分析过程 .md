...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774337920540-c32536b1-31fc-4425-85eb-c7e66d0406d6.png)

<font style="color:rgb(38, 38, 38);">单独分析这条指令，对照指令集手册：</font>

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774338126317-1e223e54-5a15-49b9-8c27-e74d1095a38d.png)
...
**逻辑 1 号寄存器 (**`**ra**`**) 的值应为 **`**0x80000172**`

+ 这个推导是清晰和正确的。`ra`的值由调用 `main`函数的那条指令设置。
+ 调用 `main`的指令位于 `0x8000016e`：`jal ra, 8000012a <main>`。这是一条跳转并链接指令，硬件会将其**下一条指令的地址**存入 `ra`寄存器。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774339065865-55d1aa5d-c100-4f3b-80cc-5fcdd95ddf6c.png)
...
**逻辑 2 号寄存器 (**`**sp**`**) 的值暂不能轻易确定，**要计算 `sp`在 `main`函数内的值，我们需要知道**进入 **`**_trm_init**`**时 **`**sp**`**的初始值**。这个值通常由系统启动代码设置，暂不花费精力往前深究。
...
因此，我们可以直接查看译码模块（Decode）的输出波形。（经过一段迷迷糊糊的查找，最终确认目标指令被注入到了下标为3的那一路译码器（decoder_3），时刻为16767）：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774339860686-48a4c0a2-c4db-4a1b-a8d7-b93ea509ccbb.png)

因此，我们直接提取该模块中下标为 3 的译码信号，即与 `io_*_3_*`相关的信号，具体如下：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774345192952-c99cc082-20fe-4bd5-b329-afc3a8e9ce63.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774343079698-02fdf73d-b3a8-4113-9f80-6e9f37bfb00c.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774343128355-8fc71d94-d48e-416f-9b8e-8d6bf056e57a.png)
...
从波形中可以看到，`fuType`的值为 `0x10000`，其独热编码的第 17 位（下标16）为高。现在来看它实际代表的意义：  
 <!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774344232024-cdc4d6ab-c8a7-4d76-8ff8-c5ca93553788.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774344482009-dc228c97-0c61-4396-a9cb-be42bc9c375f.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774345359595-b56c2e06-6044-437f-bd4a-9f9e068a4380.png)
...
+ `**instr**`：`0x00113423`，对应 `sd ra, 8(sp)`指令。
+ `**lsrc_0**`：`0x02`，源操作数0来自逻辑寄存器2号（`sp`）。
+ `**srcType_0**`：`0x1`，源操作数0的类型为寄存器。
+ `**lsrc_1**`：`0x01`，源操作数1来自逻辑寄存器1号（`ra`）。
+ `**srcType_1**`：`0x1`，源操作数1的类型为寄存器。
+ `**selImm**`：`0xe`，表示该指令为S型，立即数需按S型格式处理。
+ `**imm**`：`0x8`，指令的立即数值为8。
+ `**fuType**`：`0x10000`，功能单元类型，指示将使用存储单元（`stu`）。
+ `**fuOpType**`：`0x3`，功能单元操作类型，表示具体的64位存储操作（`sd`）。
+ `**rfWen**`：`0x0`，寄存器写使能为低，`sd`指令不写回寄存器文件。
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774925844853-0644a2c3-cf1e-448f-84d5-634a2e37c316.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774926356657-204d64e2-78da-4019-8b07-6074ec806d15.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774927039698-20f19585-fd43-4949-b847-159e4b688968.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774938590514-77f973e6-afec-43ac-8781-7b5605e25967.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774941224480-ab60f680-9fed-474a-b21e-fa02bce593fd.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774941883793-43b57777-8a81-4b0b-828f-fb1564c9ea48.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774943734198-e9c6d3bc-80bf-468e-a8b6-00dcc336c063.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774944497593-d5193878-92c0-4ce9-a061-8fd154a5576d.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774947085225-079c87a7-616c-4ed5-80e3-0a0811f39863.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1774950083170-aa69f463-f37d-451b-b954-fb1dee03c0e0.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775099186798-20d17aa9-88e6-4193-b624-5c1b5462ec65.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775099419893-a152e18c-92d3-4c0f-ad2c-5c314e7bc169.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775008184845-2a7f3c3d-ff25-4c44-86ab-c7e88d854f79.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775011085578-8158d063-e468-42f9-bc2d-f39c96a01a19.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775011549585-5e45c60d-fb7b-493f-88c0-8f2b8ab378c9.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1776926645934-07fdeb63-4913-45a0-9514-b9d064269542.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1776926655380-773095cc-74a1-4d73-b274-7d29e061cf87.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1776926661410-72af2876-0a5c-4077-864e-f8ceb56724de.png)<font style="color:#DF2A3F;">（一共19个发射队列（sta和std为一个））</font>
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775012126112-79dc4671-ec63-4aaf-9aec-70ad6987e2b2.png)

它们分别是 `**IssueQueueStaMou_1**` 和 `**IssueQueueStdMou_1**` 这两个发射队列，分别用于发射地址计算相关的操作和数据相关的操作。

## （4）发射与执行（memScheduler + DataPath + Bypass）
既然已经明确了指令将被存入** **`**IssueQueueStaMou_1**`** **和 `**IssueQueueStdMou_1**` 两个队列，我们接下来自然要拉出这两个队列中相应的信号进行观察：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775012630279-96e079f2-eeea-4029-be88-6114f39929ac.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775014139016-514c06c8-2a0b-430d-bcad-b8f09f56fae5.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775024538854-1e1a5c2b-54a4-4cb1-a301-7f9c0990fa1b.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775025599681-cb81df2d-868a-4150-bf55-7b9188c59404.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775026086234-b32ad342-bcc2-4469-9533-f5fcf0e22f67.png)

你会发现，DataPath 和 Bypass 之间没有寄存器，因此我们观察 Bypass 网络在当前周期的波形输入与输出：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775026255396-06f8dd74-62e5-4b99-9230-9d13cf19a6c5.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775026658370-45a6178e-7ef8-4b1c-b8e7-0aacd7619029.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775026947474-dced5f29-d927-41d2-a55b-688b8d405a29.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775027408530-6a596e58-9bbc-4fa7-9958-258878331b88.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775027708495-03322e3c-8a44-4d65-8425-42277fafc088.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775028162005-f1a6e5ff-a9e5-4f17-a283-2f73a63b2509.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775028256242-613a5909-6f01-4df7-96be-df3356f413ae.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775096461173-37e08fae-80c2-4728-b4cf-c20544a44684.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775097660647-bb5122cc-bc84-4d09-8691-05cfe881cddb.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775098792544-c4142b39-daa7-4e84-a5b8-aab6f822990d.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775099605881-c13b2215-7a35-4e71-9423-1541291a68fd.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775100557174-475ec0af-c8c0-4741-a1c2-48d62deaf7cf.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775100193935-9c2b8ab7-bdd8-4da9-a25e-37a6a6d1dc0c.png)

数据离开执行阶段后，便进入了上述模块。我们直接提取这个模块中对应的数据来查看：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775100314038-5ed0a0d3-2339-4670-a824-3f6f06878d89.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775100985103-fb0ce2d3-6608-447c-8886-505e0c94db31.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775630721671-d5f2036f-2943-4926-85f2-075d7374e883.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775630756702-695d8f48-dc48-44da-87ad-814d52eee872.png)

结合SQ的状态来看这个写请求的发出：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775631118364-b18636db-00c6-4fb6-9e4e-2f73a78b5ea7.png)

接着看SBuffer的情况：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775614509749-d58c95db-718f-401a-920a-128ead90894f.png)

往回trace就会发现这些玩意儿都是来自于StoreQ的，所以直接看后面某些时刻这些接口传入的信号。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775111795012-9b970317-ada8-44c6-b3c3-889bdb32e219.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775619760115-c834a1dc-0f95-42ba-8a1c-9e5290899794.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775619636299-7d311ad3-c3f2-4983-8491-78e45215c3c6.png)

对于我们这条sd指令，他是包含着两个部分的，一个是sta一个是std，所以在这俩微操作就位之后，在robEntries中的opnum马上就变了：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775619829410-27a9f7fe-673d-45b2-96d2-9f0c763f8ee3.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775629809932-001348a9-f96b-4f16-b0e5-528c7bfa4eb1.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775629963194-9a0d0a58-7a17-4c2f-8880-f3c90dda8dd6.png)
...
在这一节之前，依据的波形都是hello_world的波形：/nfs/home/wanghao/xs-env/myWaves/helloWorld
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775615927702-9b3f9b8c-7b8d-417a-bde5-b6b6fc092ee1.png)

所以直接去观察这些信号就好咯

###  （5.1）先介绍一下两者之间的整体接口
`<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">DCacheToSbufferIO</font>` 是香山处理器中**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">DCache暴露给 SBuffer</font>**的核心交互接口，承载 SBuffer 与 DCache 之间**<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">Cache 行粒度</font>**的存数请求 / 响应交互。接口继承 `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">DCacheBundle</font>`，复用 DCache 相关的基础参数（如 Cache 行大小、地址位宽、路数等）；所有信号的设计围绕 “存数请求下发→DCache 处理→响应返回” 的核心流程展开。  

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775530208179-d36f9f78-37ef-4e24-8b56-8fb9db17fac9.png)

 1. `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">val req = Flipped(Decoupled(new DCacheLineReq))</font>`

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775530835520-5283304c-36f2-4f1d-b539-ff0b0b18243c.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775530543559-878963c3-503a-4c31-b345-e3d0de9aee2b.png)

 2. `<font style="color:rgb(0, 0, 0);background-color:rgba(0, 0, 0, 0);">val main_pipe_hit_resp = ValidIO(new DCacheLineResp)</font>`

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775530879052-3ed12305-2cde-41e6-acb0-c6c63e0f8dd9.png)
...
| <font style="color:black;">命名拆解</font> | <font style="color:black;">main_pipe：DCache 的主流水线（处理常规 Cache 访问的核心流水线，区别于 refill 填充流水线）；</font><br/><font style="color:black;">hit：存数请求命中DCache（目标 Cache 行已在 DCache 中，无需从下一级存储 / 内存填充）；</font><br/><font style="color:black;">resp：DCache 返回给 SBuffer 的响应。</font> |
...
| <font style="color:black;">交互场景</font> | <font style="color:black;">对应 sd 指令的 “Cache 命中” 场景：SBuffer 发写请求→DCache 主流水线命中→main_pipe_hit_resp.valid置位→SBuffer 收到后，结束该存数请求的处理（回到空闲状态）。</font> |
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775631537116-adad6e29-6df5-47a3-ae01-c4f4361fbf09.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775616835606-ff98ceda-71f1-4e18-9f0f-88942550d37b.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775617278409-61f735c4-ed35-4633-b62e-11b5508a149c.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775617073437-cfa78cf2-5697-4f16-aaf4-090dc7f2b7fe.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775788873272-f4219fa3-f9ac-4ea2-b6bd-87c67a446ec8.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775789003351-8a2e49a0-c457-43b1-a737-18465648f262.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775789028119-a8218208-80ca-477d-af37-2e6977f3aa72.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775787701834-0bb88268-9303-4390-a4b2-f7c1787f23bc.png)

再追踪这个传入的地址信号：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775787805988-7d430068-3042-45c3-bcd9-f4f8593575ec.png)、
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775789254721-0947e725-a1d8-4f5d-a0d2-2dbbfb053ad1.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775788724084-f1e621a2-94e2-4494-babe-7df1ddc8af29.png)

当地址信号进入MainPipa流水级之后，上图所描述的内容是：将被打一拍被赋值到信号s1_req_addr中，然后在该地址中提取tag数据（[47:12]位中的数据为Tag信息），然后用index信号去找到4路way各自的对应位置的tag，去对比4路way各自的tag数据，最终的结果也就表明，当前是对比上了way0的数据的，也就是第一个way

，表明访问地址为0x80200fc0的那个请求所需要的数据目前是有效存在于cache块中的。并且我们还可以看到meta*信号的值是0x2，先查找设计手册得知这个值所代表的意义：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775789526195-258a516b-9330-46b4-a0da-aec9224e1d3c.png)



于是发现当前Cache块的状态是Trunk状态：“Trunk”通常意味着该缓存行是“主线有效”或“主路径”数据，可能表示数据已验证、未被修改、且是主流访问路径上的数据。可能表示“已确认一致性”或“共享有效”。

所以，综合以上信号，way0的hit信号被拉高，整体指示hit的信号s1_tag_match也被拉高了，指示当前请求的Cache块是命中的，可以直接对Cache进行操作。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775790530856-8d2665c7-6258-4083-b0e8-26a02779f547.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775802545894-0bb21f2b-d70d-4e38-995e-0c3bb57abb80.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775801814650-253ddb1c-5d77-4e10-a652-53e7e308ff00.png)

会发现总响应信号是和s3_hit这个周期一起拉高的。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775802268149-5e2d9173-c5bf-4dbf-96c0-aa8bbed593c2.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775802861518-c8074b78-6092-40aa-8e16-d77f75517295.png)

可以看到该信号只要在s3有效的时候，让s3_store_can_go信号有效时候就表示这条hit的Sbuffer访问Dcache的操作已经完成了，而这个信号自然在没有miss的情况下都是拉高，所以自然就在s3流水级拉高了。
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775803610130-90cd01c5-3ebd-4aac-aaaa-ed66a7333609.png)

在io流水级和s1流水级，数据信号只是传递，没有其他的用途。但是一旦数据信号到达s2流水级成为s2_req_store_data这个数据，就有其他的用途或走向了：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775803398872-c23a45f6-8308-4f98-982f-0a0f176f8af3.png)

会根据miss信号的值给另外的一个信号*new_data*进行赋值，实际上，很容易猜出这个新赋值的信号的意义是什么，他表示将要往DataArray写的数据，而能往DataArray写的数据实际上总共就两个来源，要么是Store操作进行写入的值，要么就是充填更新Dcache时从L2Cache拿过来的值。这时再往上看那一串代码，实际上就是在做了一个选择操作，在这两个值中间选一个。那么很显然，在我们这条hit了的请求下 ，这时候请求进入s2流水级时要进行的操作肯定选第一个情况，往DataArray中写值。所以在上图中，会看到*new_data*信号被赋上了s2_req_store_data的值。
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775804664582-cef5c34c-3fe2-4487-a754-cb23a7549190.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775804784364-4fc943a5-4173-4a99-8139-e6270dc423e2.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775805497301-ebf2218c-fa69-4002-ac86-2a3b0e967066.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775805607667-11847a4c-d0de-43b4-9499-a47483a3eb1a.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775806189127-66ad538b-8436-4602-80ba-85392aded38d.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775806313504-ef87ac0f-ac3e-4628-a33c-bd41e2322d2f.png)

写使能的来源：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775806387501-1b8885c7-d9ea-46bc-aed8-0fb27ee78abf.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775806458420-92a3c50a-3a92-4410-bf2d-990faac28108.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775806508218-3680eae6-8db3-40b7-ab5e-64757abf9408.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775809895692-f0433368-4040-4042-8534-a6558017f952.png)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775810448913-07e75112-a964-4d64-a559-47178cd1db1b.png)

发现实际上就是过了两个周期就往MQ里面送发请求了。

那么最重要的肯定就是发请求的使能信号io_miss_req_valid信号的使能生成逻辑了，往前跟踪：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775810503020-ab4dfcb1-3c07-44a3-8de4-a8ad32fa11dc.png)

发现用的s2_can_go_to_mq信号的逻辑来主要决定的,继续往前追踪：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775810583324-081b9093-e66a-48df-bc0b-dc979df045a1.png)

发现是由这么一套逻辑生成的，我们还是主要着眼于看见“s1_tag_match”这个信号，发现当没有出现hit的信号的时候，也就主要会把这个值拉高。

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/66154403/1775810815631-6aca9d7f-01c0-45ac-bd4d-0e6d21d8ce7e.png)

所以，对MQ发送请求的使能信号的过程大致就是以上这种过程。（s1_req_miss的实际所代表的意义暂不明确）
