...
3. **Import 检查**：禁止块导入、禁止通配符导入（仅 chisel3._、chisel3.util._ 豁免）。
...
+ 流水级信号：s1_valid、s2_ready（s0/s1/s2 前缀）
+ 调试/性能信号：debug_xxx、perf_xxx（最终交付 RTL 需剔除）
+ 多组 IO 区分：io_xxx（尽量少用）
+ 状态机标识：s_idle、s_busy
...
+ 同一语义全程统一：misalign 系列变量禁止 mis_align、misAlign、misalign_buf 混搭
...
+ 禁止非法通配符：除 chisel3._、chisel3.util._ 外，所有 xxx._ 导入禁止使用
...
# 参考素材
1. Java: [https://google.github.io/styleguide/javaguide.html](https://google.github.io/styleguide/javaguide.html)
2. Scala: [https://docs.scala-lang.org/style/](https://docs.scala-lang.org/style/)
3. Chisel: [https://www.chisel-lang.org/docs/developers/style](https://www.chisel-lang.org/docs/developers/style)
